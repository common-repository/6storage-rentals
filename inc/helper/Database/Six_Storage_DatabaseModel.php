...
namespace SixStorageOnline\helper\Database;

use ArrayObject;
use SixStorageOnline\helper\Supports\Six_Storage_Data;
use SixStorageOnline\helper\Supports\Six_Storage_DataStoreInterface;
use SixStorageOnline\helper\Supports\QueryBuilder;
use SixStorageOnline\helper\Supports\Six_Storage_Cacheable;
use SixStorageOnline\helper\Supports\Six_Storage_TableInfo;
 ...
 */
abstract class Six_Storage_DatabaseModel extends Six_Storage_Data implements Six_Storage_DataStoreInterface {

	use Six_Storage_Cacheable, Six_Storage_TableInfo;
	 ...
	 * @var string
	 */
	protected $cache_group = 'six_storage';
	 ...
	 *
	 * @return array|static
	 * @see Six_Storage_DatabaseModel::batch_update();
	 */
	 ...
	 *
	 * @return int[]
	 * @see Six_Storage_DatabaseModel::batch_create();
	 */
...
//		     ->first();
	...
	public function batch_update( array $data ) {
		...
		foreach ( $data as $index => $item ) {
			...
			$default = $default instanceof Six_Storage_Data ? $default->data : $default;
			list( $_data, $_format ) = $this->format_item_for_db( $item, $default, $current_time );
	...
	public function read( $data ) {
		if ( $data instanceof Six_Storage_Data ) {
			return $data->data;
		}

		if ( is_numeric( $data ) ) {
			$item = $this->find_by_id( $data );
			if ( $item instanceof Six_Storage_Data ) {
				return $item->data;
			}
